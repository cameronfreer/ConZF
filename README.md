...
```lean
def AccHyp : Prop := ∀ R : Path → Path → Prop, SWF R [] → ¬¬Acc R []

theorem PSet.con_ZF (hacc : AccHyp) : Con ZF
-- 'PSet.con_ZF' does not depend on any axioms
theorem PSet.accHyp_of_not_not_em (h : ¬¬∀ p : Prop, p ∨ ¬p) : AccHyp
```

`SWF R p` says that `p` is in the well-founded part of `R` as far as stable predicates can
tell. The hypothesis is an instance of the double negation shift; it follows from `¬¬EM`, and
we do not know whether it is provable outright.
...
| `ConZF/Mat.lean` | the recursion `F` (an `Acc.rec` on paths), coherent assignments, `materialize` (the value), `SWF` |
| `ConZF/Repl.lean` | the glued tree, `swf_root`, `replacement` from `¬¬Acc` of the root |
