...
| `ConZF/Graph/Transport.lean`, `Monotone.lean` | finite-name transport along a bisimulation; bounded monotone induction on the points of a graph set: the history by predicate recursion, persistence after stationarity, Hartogs-forced convergence, the native stopping cut and least closed endpoint; the adjoin-a-set instance and the predecessor-rule benchmark on an ordinal `η`, whose stopping cut is `η` |
